...
class TorchCNN4Images:
    ...
    def __init__(self, input_shape, n_output, learning_rate):
        self.model = TorchConvNet(input_dims=input_shape, n_output=n_output)
    ...
    def train(self,
        ...
        for epoch in range(n_epochs):
            total_loss = 0
            # I think I have to zero out the gradients up hear, outside of the minibatch loop... or maybe not
            # self.model.zero_grad()
            # optimizer.zero_grad()  # Do we need both model and optimizer zero_grad() ???
            for i in range(0, len(X_train), batch_size):
                ...
                # Do we need both model and optimizer zero_grad() ???
                # self.model.zero_grad()
                # Backpropagation:
                optimizer.zero_grad()
                loss.backward()
    ...
    def validate(self, X_val, y_val, loss_function):
        ...
        # accuracy = (predictions == y_val).mean()  # Might be a bug in this line...
    ...
    def save_model(self, model_filename, rel_path='models'):
        ...
        torch.save(self.model.state_dict(), f'{PROJECT_ROOT}/{rel_path}/{model_filename}')

    def load_model(self, model_filename, rel_path='models'):
        model_filename += '_torch.pt'  # Specify using Torch
        state_dict = torch.load(f'{PROJECT_ROOT}/{rel_path}/{model_filename}')
